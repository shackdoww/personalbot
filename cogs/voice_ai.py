...
class VoiceAI(commands.Cog):
    ...
    async def talk(self, interaction: discord.Interaction):
        ...
        if not interaction.user.voice or not interaction.user.voice.channel:
            ...
            return
        if not self.relay_url or not self.relay_secret:
            await interaction.response.send_message("Voice AI relay is not configured yet.")
            return

        await interaction.response.defer()
        try:
            voice = await self.start_voice(interaction.guild, interaction.user.voice.channel)
            await interaction.followup.send(
                f"🗣️ I'm listening in **{voice.channel.name}**. Talk normally and I'll respond."
            )
        except Exception as exc:
            await interaction.followup.send(f"❌ Could not start voice AI: `{type(exc).__name__}`")
    ...
    @app_commands.command(name="shutup", description="Stop the AI voice assistant and leave VC.")
